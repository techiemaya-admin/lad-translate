...
| Live transcript (`console/transcript.py`) | Done, 17 tests; source and every language with per-language latency |
| Transcript download (`console/exports.py`) | Done; txt / json / srt / vtt for a whole session |
...
### Downloading the whole session

The panel tails a talk; the download row under it takes all of it. Pick a
session - the live one, or any earlier one in that room - and a format:

    txt    everything, readable: source line, each language under it
    json   the rows as stored, timings and latencies included
    srt    subtitles, one language per file
    vtt    the same for the web

Subtitles can also be the speaker themselves (`source`), for cutting the
original. **The times are positions in the speaker's audio**, so an SRT
dropped onto `source.wav` from a recording is correct; the translated WAVs in
that recording sit where each phrase was handed to playout, a second or so
earlier than the audience heard it, and are not on the same clock. The
console says so under the picker, because a producer would otherwise find it
by eye and then not trust the rest.

The download is unbounded, unlike the live panel: truncating it would lose
the beginning of the keynote, which is the part a client reads first. A
session id from a different room is refused rather than served, so a stale id
cannot quietly hand over the wrong talk.
